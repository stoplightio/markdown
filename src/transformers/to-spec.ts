...
function captureAnnotations(node: UnistTypes.INode | undefined): SMdast.IAnnotations {
  if (!node || !node.value) {
    return {};
  }
  ...
  if (node.type === 'html' && node.value.startsWith('<!--') && node.value.endsWith('-->')) {
    ...
    const raw = node.value.substr('<!--'.length, node.value.length - '-->'.length - '<!--'.length).trim();
...
function processNode(node: UnistTypes.INode, anno: SMdast.IAnnotations | null): UnistTypes.INode {
  const { type } = node;
...
export const toSpec = (root: Mdast.IRoot): SMdast.IRoot => {
  ...
  const processed: UnistTypes.INode[] = [];
